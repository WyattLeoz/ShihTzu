    ...
    <div className="p-6">
      ...
      <div className="flex items-center gap-4 mb-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(['all', ...INCIDENT_TYPES.map((t) => t.value)] as const).map((type) => (
            <button
              key={type}
        ...
        <button
          onClick={() =>
            setSortBy((s) =>
              s === 'created_at' ? 'severity' : s === 'severity' ? 'ai_score' : 'created_at'
            )
          }
        ...
        <table className="w-full">
          <thead className="bg-paper sticky top-0">
            <tr>
              ...
              <th className="w-[110px] px-4 py-3 text-left text-xs font-mono font-semibold text-ink-muted uppercase">
                Ticket
              </th>
              <th className="px-4 py-3 text-left text-xs font-mono font-semibold text-ink-muted uppercase">
                Incident
              </th>
              <th className="w-[90px] px-4 py-3 text-left text-xs font-mono font-semibold text-ink-muted uppercase">
                AI Score
              </th>
              <th className="w-[110px] px-4 py-3 text-left text-xs font-mono font-semibold text-ink-muted uppercase">
                Status
          ...
          <tbody>
            {isLoading ? (
              ...
              incidents.map((incident) => {
                ...
                const aiConfidence = incident.aiTriageData?.options?.[0]?.confidence;
                const isOldOpen =
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`font-mono text-xs ${
                          isOldOpen ? 'text-red' : 'text-ink-muted'
                        }`}
                      >
